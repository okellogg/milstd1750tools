...
static const struct {
		    ...
		    } comtab[] =
 {
   ...
   { "psload <file>",             si_pslo,   "load IBM UC Program Store file",
       ...
       "Download) format." },
   { "pload <file>",              si_pload,  "load ASL P-format binary file",
       "For more information on Alfred Arnold's ASL and the P format, see:\n"
       "ftp://ftp.kfa-juelich.de/pub/msdos/assemblers/" },
  ...
  if (i_o != NULL)
      ...
      if (i_o > str)
	{
	  if ((hexdigit = xtoi (*str)) == -1)
	    return error ("syntax problem, expecting Address State");
	  as = (ushort) hexdigit;
	}
      str = i_o + 1;
    }
...
si_disasm (int argc, char *argv[])
{
  ...
  if (argc > 1)
    {
      if (parse_address (argv[1], &disasm_addr) != OKAY)
  ...
  else if (disasm_addr < 0L)
    disasm_addr = get_phys_address (CODE, simreg.sw & 0xF, simreg.ic);
  ...
  for (i = 0; i < length; i++)
    {
      if (sys_int (1L))
	return (INTERRUPT);
      if ((sym = find_labelname (disasm_addr)) != NULL)
	lprintf ("                      %s\n", sym);
      lprintf ("%05lX:     ", disasm_addr);
      if (! peek (disasm_addr, &words[0]))
	{
	  verbose = verbose_save;
	  return error ("<no code loaded here>");
	}
      peek (disasm_addr + 1, &words[1]);
