  ...
  switch (vartyp)
    ...
    case VAR_LONG:
      ...
      switch (operation)
        ...
        elsecase ARI_SUB:
          {
            unsigned long lop0 = ((ulong) operand0[0] << 16)
                               | ((ulong) operand0[1] & 0xFFFFL);
                     long lop1 = ((ulong) operand1[0] << 16)
                               | ((ulong) operand1[1] & 0xFFFFL);

            unsigned long long llop0 = (unsigned long long) lop0;
                     long long llop1 = - (long long) lop1;
            unsigned long long llaccu;

	    llop1 = -llop1 & 0xFFFFFFFFLL;
            llaccu = llop0 + (unsigned long long) llop1;

            if (llaccu & 0x100000000LL)
              simreg.sw |= CS_CARRY;

            if ((llop0 & 0x80000000LL) != (llop1 & 0x80000000LL) &&     
                (llop0 & 0x80000000LL) != (llaccu & 0x80000000LL))
              {
                simreg.pir |= INTR_FIXOFL;
...
#define MAX_LONG  0x7FFFFFFF
      {  /* Not so nice and easy if we don't have a larger data type. */
        long lop0 = ((long) operand0[0] << 16) | ((long) operand0[1] & 0xFFFF);
        long lop1 = ((long) operand1[0] << 16) | ((long) operand1[1] & 0xFFFF);
        long laccu;
        int opsign;
